...
if __name__ == "__main__":
    ...
    if options.proxy:
        handlers.append(urllib.request.ProxyHandler({'http': 'http://' + options.proxy}))
    ...
    if options.basicauth:
        ...
        basic_auth_encoded = base64.b64encode(credentials).decode('ascii')
    ...
    html_files = re.findall("([A-Z0-9]{30,35})", the_page.decode('utf-8'))
    ...
    for html_file in html_files:
        ...
        req = urllib.request.Request(cache_html)
        ...
        for line in the_page:
            ...
            rpc_method_match = re.match("^function \w+\(.*method\:([A-Za-z0-9_\$]+),.*$", line.decode('utf-8'))
